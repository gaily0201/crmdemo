...
<body>
	<form name="ActionForm" method="post"
		...
		<div class="mtitle">
			...
			<span onClick="" class="menu_selected">共享设置</span> <span
				onClick="goCancel();" class="menu_noselect">取消共享</span> <span
				onClick="goView();" class="menu_noselect">共享查看</span>
		</div>
		<br />
		<div class="control">
			...
			<button type='button' class='button'
				...
				<img
					src="${pageContext.request.contextPath}/ui/images/button/mokuaiqx.png"
					border='0' align='absmiddle'>&nbsp;模块全选
			</button>
			<button type='button' class='button'
				onMouseOver="this.className='button_over';"
				onMouseOut="this.className='button';" onClick="unSelectAllBox()">
				<img
					src="${pageContext.request.contextPath}/ui/images/button/mokuaiqbx.png"
					border='0' align='absmiddle'>&nbsp;模块全不选
			</button>
			<button type='button' class='button'
				onMouseOver="this.className='button_over';"
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			bgcolor="#6A82A8">
			<tr>
				<td align="center" height="25"><span class="Tabfont">${company.name}(客户)待共享的模块</span>
				</td>
			</tr>
		</table>
		<div id="border" class="border" style="padding:3px">
			<fieldset style='padding:5px;clear:left;'>
				<legend> 选择模块： </legend>
				<div>
					<input type='checkbox' class='checkbox' name='s_module' checked='checked'
						value='c_company' id='c_company' onclick='do_select(this.id)'>客户资料
				</div>
				&nbsp;
			</fieldset>
		...
		<div class="control">
			<button type='button' class='button'
				...
				<img
					src="${pageContext.request.contextPath}/ui/images/button/yonghuqx.png"
					border='0' align='absmiddle'>&nbsp;用户全选
			</button>
			<button type='button' class='button'
				onMouseOver="this.className='button_over';"
				onMouseOut="this.className='button';" onClick="unSelectAllUser()">
				<img
					src="${pageContext.request.contextPath}/ui/images/button/yonghuqbx.png"
					border='0' align='absmiddle'>&nbsp;用户全不选
			</button>
		</div>
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td align="center" height="25"><span class="Tabfont">${company.name}(客户)待共享的用户</span>
				</td>
			</tr>
		</table>
		...
		<c:forEach items="${sysUserGroups}" var="sysUserGroup">
			<div class="border" style="padding:3px">
				<fieldset style='padding:5px;clear:left;'>
					...
					<c:if test="${sysUserGroup.id==sysUser.sysUserGroup.id}">	
							<div>
								<input type='checkbox' class='checkbox' name='uid' value='${sysUser.id}'
									id=uid'${sysUser.id}' onclick='do_select(this.id)'>${sysUser.cnname}
							</div>
							&nbsp;
					</c:if>
					</c:forEach>
