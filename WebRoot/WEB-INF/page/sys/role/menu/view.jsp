			...
			<tr>
				<td align="center" height="25"><span
					style="color:#FFFFFF; font-weight:bold">操作权限组：<s:property
							value="%{#request.sysRole.name}" /> </span>
				</td>
			</tr>
		</table>
		<div class="border" style="padding:3px">
			<c:if test="${! empty sysMenus}">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td><c:forEach items="${sysMenus}" var="sysMenu">
								<c:if test="${sysMenu.id.menuModule==sysMenu.id.menuPrivilege}">
									<fieldset style='padding:5px;clear:left;'>
										...
										<c:if test="${! empty sysMenuPrivileges }">
											<c:forEach items="${sysMenuPrivileges}"
												var="sysMenuPrivilege">
												<c:if
													test="${sysMenu.id.menuModule==sysMenuPrivilege.id.menuModule
														&&sysMenu.id.menuPrivilege==sysMenuPrivilege.id.menuPrivilege}">
													<c:set value="checked" var="xx" />
												</c:if>
											</c:forEach>
										</c:if>
										<legend>
											...
											<c:if
												...
												<c:if test="${! empty sysMenuPrivileges }">
													<c:forEach items="${sysMenuPrivileges}"
														var="sysMenuPrivilege">
														<c:if
															test="${sysMenuSub.id.menuModule==sysMenuPrivilege.id.menuModule
														&&sysMenuSub.id.menuPrivilege==sysMenuPrivilege.id.menuPrivilege}">
															<c:set value="checked" var="xxx" />
														</c:if>
													</c:forEach>
												</c:if>
