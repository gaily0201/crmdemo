		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<tr>
				<td align="center" height="25"><span
					style="color:#FFFFFF; font-weight:bold">操作权限组：${sysRole.name}</span>
				</td>
			</tr>
		</table>
		<div class="border" style="padding:3px">
			<c:if test="${! empty requestScope.sysPopedoms}">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td><c:forEach items="${requestScope.sysPopedoms}"
								var="sysPopedomSup">
								<c:if
									test="${sysPopedomSup.id.popedomModule==sysPopedomSup.id.popedomPrivilege}">
									<fieldset style='padding:5px;clear:left;'>

										<c:forEach items="${sysPopedomPrivileges}"
											var="sysPopedomPrivilege">
											<c:if
												test="${sysPopedomSup.id.popedomModule==sysPopedomPrivilege.id.popedomModule
										...
										<c:forEach items="${requestScope.sysPopedoms}"
											var="sysPopedomSub">
											<c:if
												test="${sysPopedomSup.id.popedomModule==sysPopedomSub.id.popedomModule
														&&sysPopedomSub.id.popedomPrivilege!=sysPopedomSub.id.popedomModule}">
												<c:forEach items="${sysPopedomPrivileges}"
													var="sysPopedomPrivilege">
													<c:if
														test="${sysPopedomSub.id.popedomModule==sysPopedomPrivilege.id.popedomModule
															&&sysPopedomSub.id.popedomPrivilege==sysPopedomPrivilege.id.popedomPrivilege}">
														<c:set value="checked" var="yy" scope="page" />
													</c:if>
												</c:forEach>
