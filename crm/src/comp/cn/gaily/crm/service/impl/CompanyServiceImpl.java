...
public class CompanyServiceImpl implements CompanyService{
	...
	public String getCompanyCodeByTabName(String tabName) {
		...
		if(sysCodeRule.getAvailable().equals("Y")){
			...
			//生成客户编码
			String code = sysCodeRule.getAreaPrefix()+"-"+DateFormatUtils.format(new Date(), sysCodeRule.getAreaTime())+"-"+firstGlideNumber;
			//修改编码规则表
			...
			if(curDate.equals(sysCurDate)){
				...
				//生成客户编码
				String code = sysCodeRule.getAreaPrefix()+"-"+DateFormatUtils.format(new Date(), sysCodeRule.getAreaTime())+"-"+nextseq	;
				//修改代码规则
				sysCodeRule.setNextseq(nextGlideNumber);
				sysCodeRule.setCurrentCode(code);
				sysCodeRuleDao.update(sysCodeRule);
	...
	@Override
	@Transactional(readOnly=true)
	public List<Company> findCompanyByCondition(SysUser curSysuser, CompanySearch companySearch) {
		...
		if(curSysuser!=null&&companySearch!=null){
			...
			return companyDao.findObjectsByConditionWithNoPage(whereHql, params, orderby);
		}
		return null;
	}
			...
			if("c_company".equals(s_module)){//模块名称为客户模块
				...
				if(company!=null){
					...
					if('N'==company.getShareFlag()){
						...
						company.setShareFlag('Y');
						company.setShareIds(company.getShareIds()+sb.toString());
						companyDao.update(company);
					}
				}
			}
			...
			if(company!=null){
				...
				if(StringUtils.isNotBlank(shareIds)){
					...
					if(ids!=null&&ids.length>0){
						...
						whereHql.append(")");
						return sysUserDao.findObjectsByConditionWithNoPage(whereHql.toString(), params.toArray());
					}
				}
			}
		}
	...
	public List<Company> findSharedCompany(SysUser curSysuser) {
		...
		if(id!=null){
			...
			params.add('Y');
			List<Company> list =  companyDao.findObjectsByConditionWithNoPage(whereHql, params.toArray());
			if(list!=null&&list.size()>0){
				for(int i=0;i<list.size();i++){
					Company company = list.get(i);
					String shareIds = company.getShareIds();
					if(StringUtils.isNotBlank(shareIds)&&shareIds.contains("#"+id+"#")){
